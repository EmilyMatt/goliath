...
impl GoliathServer {
    pub(crate) async fn await_connection(&mut self) -> GoliathVehicleResult<GoliathVehicleSession> {
        ...
        let ws_conn = accept_async(MaybeTlsStream::Plain(new_connection))
            .await
            .map_err(Box::new)?;
        GoliathVehicleSession::try_new(addr, ws_conn, ZedCamCaps::NOHD15, EncoderType::V4L2)
    }
